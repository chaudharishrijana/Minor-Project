...
router.get('/image/:id', async (req, res) => {
    try {
        const id = req.params.id;
        // Log the received artwork ID
        console.log('Received artwork ID:', id);

        // Find artwork in the database based on the provided ID
        const artwork = await artworks.findByPk(id);

        // Log the artwork retrieved from the database
        console.log('Retrieved artwork:', artwork);

        // Check if artwork exists
        if (artwork && artwork.image) {
            // Construct the image path
            const imagePath = path.join(__dirname,'uploads', artwork.image);

            // Log the constructed image path
            console.log('Constructed image path:', imagePath);
            console.log('Does the image file exist?', fs.existsSync(imagePath));
            ...
            if (fs.existsSync(imagePath)) {
                ...
                // If image file not found, send 404 error
                res.status(404).json({ error: 'Image file not found' });
            }
        } else {
            console.log('Artwork not found');
            // If artwork not found or artwork has no image, send 404 error
